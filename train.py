...
def extract_features(parsed_args, train_df, val_df, feature_names, embeddings_path, conceptnet_path):
    ...
    if selected_embedding == 'glove':
        ...
        print('EXTRACTING GLOVE EMBEDDINGS FROM TRAINING SET')
        train_df = generate_glove_embeddings(parsed_args, train_df, feature_names, embeddings_path)

        print('EXTRACTING GOLVE EMBEDDINGS FROM VALIDATION SET')
...
if __name__ == '__main__':
    ...
    # Extract features
    train_df_feat, val_df_feat = extract_features(parsed_args, train_df, val_df, feature_names, embeddings_path, conceptnet_path)
